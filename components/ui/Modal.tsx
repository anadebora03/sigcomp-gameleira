...
export default function Modal({ title, onClose, children, wide }: Props) {
  ...
  const overlay: React.CSSProperties = {
    ...
    zIndex:99999, background:'rgba(0,0,0,.75)',
    display:'flex', alignItems:'flex-start', justifyContent:'center',
    overflowY:'auto', padding:'40px 16px 60px', boxSizing:'border-box',
  }

  return createPortal(
    <div style={overlay} onMouseDown={e => { if (e.target === e.currentTarget) onClose() }}>
      <div style={{
        background:'var(--card)', borderRadius:18, width:'100%',
        maxWidth: wide ? 820 : 600, flexShrink:0,
        boxShadow:'0 24px 80px rgba(0,0,0,.35)', position:'relative', zIndex:1,
      }}>
        <div style={{
          ...
          <button onClick={onClose} style={{
            ...
            background:'var(--inp)', cursor:'pointer', display:'flex',
