...
export default function App(){
  ...
  const alertCount=store.oficios.filter(o=>(o.prazo&&new Date(o.prazo)<new Date())||o.status==='pendente').length
  ...
  return(
    <div style={{fontFamily:'system-ui,sans-serif',display:'flex',height:'100dvh',overflow:'hidden',background:'var(--bg)'}}>
      ...
      <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',minWidth:0}}>
        <Header page={page} mob={mob} dark={dark} setDark={setDark} sideOpen={sideOpen} onOpenSide={()=>setSideOpen(true)} onOpenMob={()=>setMobMenu(true)} onLogout={()=>setLoggedIn(false)}/>
        <main style={{flex:1,overflowY:'auto',padding:mob?'12px 11px':'18px 22px',paddingBottom:mob?74:18}}>
          <div style={{animation:'fadeUp .3s ease both',maxWidth:1400,margin:'0 auto'}}>
            ...
            {page==='secretarias'  && <SecretariasPage oficios={store.oficios} processos={store.processos}/>}
            ...
            {page==='usuarios'     && <UsuariosPage  usuarios={store.usuarios} saveUsuario={store.saveUsuario} deleteUsuario={store.deleteUsuario} toast={showToast}/>}
