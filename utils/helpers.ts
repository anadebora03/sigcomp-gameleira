...
export const gS=(id?:number)=>SECS.find(s=>s.id===id)||{sigla:'?',cor:'#94a3b8',nome:'--'}
...
export const openFile=(dataUrl:string,nome:string)=>{
  try{
    ...
    const mime=nome.toLowerCase().endsWith('.pdf')?'application/pdf':dataUrl.split(';')[0].split(':')[1]||'application/octet-stream'
